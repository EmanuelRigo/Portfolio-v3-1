...
export default function ProjectsSection({ onProjectClick }: ProjectsSectionProps) {
  ...
  return (
    <>
      ...
      <section id="previous" className="py-16 px-6 lg:px-16 border-t border-border-subtle/30 bg-surface-slate/10">
        <div className="max-w-5xl mx-auto font-sans">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {PROJECTS.filter(p => p.category === 'previous').map((project) => (
              <ProjectCard 
                key={project.id} 
                project={project} 
                onClick={() => onProjectClick(project)} 
