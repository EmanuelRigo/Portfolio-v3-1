...
import { Briefcase, GraduationCap, Cpu, Sparkles, MessageSquare } from 'lucide-react';
...
export default function ResumeSection({ onLaunchChat }: ResumeSectionProps) {
  ...
  return (
    <section id="resume" className="py-16 px-6 lg:px-16 border-t border-border-subtle/30 bg-surface-slate/10">
          ...
          <div className="space-y-8 font-sans">
            <div className="bg-surface-slate border border-border-subtle p-6 rounded-2xl space-y-4">
              ...
              <div className="space-y-4">
                <div>
                  <span className="text-[10px] text-text-muted uppercase tracking-wider block mb-1.5 font-bold font-mono">Frontend</span>
                  <div className="flex flex-wrap gap-1.5">
                    {['React', 'Next.js', 'TypeScript', 'Tailwind v4', 'Motion', 'Zustand', 'Context API'].map(tag => (
                      <span key={tag} className="px-2 py-1 rounded bg-surface-charcoal border border-border-subtle text-xs text-on-surface-variant font-medium">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  <span className="text-[10px] text-text-muted uppercase tracking-wider block mb-1.5 font-bold font-mono">Backend y Storage</span>
                  <div className="flex flex-wrap gap-1.5">
                    {['Node.js', 'Express', 'Prisma ORM', 'PostgreSQL', 'Firebase Firestore', 'REST APIs'].map(tag => (
                      <span key={tag} className="px-2 py-1 rounded bg-surface-charcoal border border-border-subtle text-xs text-on-surface-variant font-medium">
                        {tag}
                      </span>
                    ))}
                ...
                <div>
                  <span className="text-[10px] text-text-muted uppercase tracking-wider block mb-1.5 font-bold font-mono">Entorno</span>
                  <div className="flex flex-wrap gap-1.5">
                    {['Git / GitHub', 'Docker', 'Vite', 'npm / bun', 'Criptografía firma'].map(tag => (
                      <span key={tag} className="px-2 py-1 rounded bg-surface-charcoal border border-border-subtle text-xs text-on-surface-variant font-medium">
                        {tag}
                      </span>
                    ))}
            ...
            <div className="bg-gradient-to-br from-surface-slate to-surface-charcoal border border-primary-container/30 p-6 rounded-2xl relative overflow-hidden flex flex-col justify-between">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-primary-container uppercase tracking-widest font-mono">IA INTEGRADA</span>
                </div>
