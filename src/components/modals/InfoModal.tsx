          ...
          >
            ...
            >
              ...
              <div className="w-full md:w-7/12 flex flex-col justify-between p-6 sm:p-8 space-y-6 max-h-[70vh] md:max-h-[500px] lg:max-h-[600px] overflow-y-auto custom-scrollbar relative">
                ...
                <button
                  onClick={onCloseProject}
                  className="absolute top-4 right-4 bg-surface-charcoal/80 hover:bg-surface-charcoal backdrop-blur-md border border-border-subtle hover:border-primary-container p-2 rounded-lg transition-all text-on-surface cursor-pointer z-10"
                >
                  <X className="w-4 h-4" />
                </button>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 text-xs sm:text-sm text-on-surface-variant leading-relaxed">
                    ...
                    {project.architecture && (
                      <div className="p-4 rounded-lg bg-surface-charcoal border border-border-subtle/70 space-y-1">
                        <span className="text-[10px] text-primary-container font-mono uppercase tracking-widest font-bold">
                          {messages.Projects["modal"]["architectureLabel"]}
                        </span>
                  ...
                  {project.features && project.features.length > 0 && (
                    <div className="space-y-3 pt-2">
                      <h5 className="font-serif text-sm font-bold text-on-surface flex items-center gap-1.5">
                        ...
                        <span>
                          {messages.Projects["modal"]["featuresTitle"]}
                        </span>
                      </h5>
                ...
                <div className="pt-6 border-t border-border-subtle/30 flex flex-wrap gap-3 mt-auto">
                  ...
                  {project.githubUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-center gap-2 bg-surface-charcoal border border-border-subtle hover:border-primary-container/50 px-5 py-3 rounded-lg font-sans text-xs font-bold transition-all cursor-pointer"
                    >
                      <Terminal className="w-3.5 h-3.5 text-primary-container" />
                      <span>
                        {messages.Projects["modal"]["githubCodeButton"]}
                      </span>
                    </a>
                  )}
                </div>
        ...
        {certificate && (
          ...
          >
            <motion.div
              ...
              className="bg-surface-slate border border-border-subtle w-full max-w-lg rounded-lg overflow-hidden shadow-2xl p-6 sm:p-8 space-y-6"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="flex items-start justify-between border-b border-border-subtle/50 pb-4">
                <div className="flex items-center gap-3">
                  <div className="bg-surface-charcoal/80 p-3.5 rounded-lg border border-border-subtle">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 p-4 rounded-lg bg-surface-charcoal border border-border-subtle text-xs">
                  <div>
                    <span className="text-[9px] text-text-muted block uppercase font-bold mb-0.5">
                      {messages.Certificates["modal"]["academicIssuer"]}
                    </span>
                  ...
                  <div>
                    <span className="text-[9px] text-text-muted block uppercase font-bold mb-0.5">
                      {messages.Certificates["modal"]["issueDate"]}
                    </span>
                    <span className="text-on-surface font-semibold">
                      {certificate.date}
                    </span>
                  ...
                  <div className="col-span-2 border-t border-border-subtle/40 pt-2 font-mono">
                    <span className="text-[9px] text-text-muted block uppercase font-bold mb-0.5 font-sans">
                      {messages.Certificates["modal"]["credentialIdLabel"]}
                    </span>
              ...
              <div className="space-y-2">
                <span className="text-[10px] text-text-muted uppercase font-bold tracking-wider block">
                  {messages.Certificates["modal"]["skillsEvaluated"]}
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {certificate.skillsEarned.map((tag) => (
                    <span
                      key={tag}
                      className="bg-surface-charcoal border border-border-subtle/80 px-2.5 py-1 rounded text-xs text-on-surface-variant flex items-center gap-1"
                    >
              ...
              <div className="pt-4 border-t border-border-subtle/30 space-y-4">
                <AnimatePresence mode="wait">
                  {isValidating ? (
                    ...
                    <motion.div
                      ...
                      className="p-4 rounded-lg bg-emerald-500/5 border border-emerald-500/30 flex items-center gap-3"
                    >
                      <ShieldCheck className="w-7 h-7 text-emerald-400 flex-shrink-0 animate-pulse" />
                      <div>
                        <p className="text-xs font-bold text-emerald-400">
                          {messages.Certificates["modal"]["credentialValid"]}
                        </p>
                        <p className="text-[11px] text-text-muted">
                          {
                            messages.Certificates["modal"][
                              "credentialValidDescription"
                            ]
                          }
                        </p>
                      </div>
                ...
                {!validationSuccess && !isValidating && (
                  <button
                    ...
                    className="w-full py-3 bg-primary-container text-on-primary hover:shadow-[0_0_15px_rgba(250,204,21,0.25)] rounded-lg font-sans text-xs font-bold tracking-wider transition-all cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="w-full py-3 bg-surface-charcoal text-text-muted hover:text-white rounded-lg font-sans text-xs transition-all border border-border-subtle cursor-pointer"
                >
